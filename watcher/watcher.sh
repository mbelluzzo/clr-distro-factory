...
log "Downstream Stage" "${STAGING_DIR}"
log_line
...
log "Downstream version"
if [[ -z $DS_LATEST ]]; then
    log_line "First Mix! It's Release Time!" 1
    exit 1
fi
log_line "${DS_UP_VERSION} ${DS_DOWN_VERSION}" 1

if (( DS_UP_VERSION < CLR_LATEST )); then
    log "Upstream has a new release" "It's Release Time!"
    exit 1
fi
...
if result=$(koji_cmd list-tagged --latest --quiet "${KOJI_TAG}"); then
    ...
    warn "Failed to fetch Downstream Package List" "Assuming empty."
fi

if ! diff "${TMP_CURR_LIST}" "${TMP_PREV_LIST}"; then
    log "New custom content" "It's Release Time!"
    ret=1
else
